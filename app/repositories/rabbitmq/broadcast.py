...
class BroadcastRepository(AbstractRabbitMQRepository):
    ...
    async def _get_exchange(self) -> AbstractExchange:
        if self._exchange is None:
            self._exchange = await self._declare_exchange(settings.rabbitmq.WS_BROADCAST_EXCHANGE)
        return self._exchange

    async def publish(self, event: WsBroadcastEvent) -> None:
        exchange = await self._get_exchange()
        await self._publish(exchange, orjson.dumps(event.model_dump(mode="json")))
